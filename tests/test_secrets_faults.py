...
class TestEnvSecretsBackend:
    ...
    def test_get_with_overridden_prefix(self) -> None:
        key = "API_KEY"
        expected = "sk-abc123"
        with patch.dict(os.environ, {"CUSTOM_SECRET_API_KEY": expected}, clear=False):
    ...
    def test_get_normalizes_key_path_separators(self) -> None:
        key = "vault/path/key"
        expected = "my-value"
        with patch.dict(os.environ, {"UNDERWRITE_SECRET_VAULT_PATH_KEY": expected}, clear=False):
            backend = EnvSecretsBackend()
            result = backend.get(key)
        assert result == expected
    ...
    def test_get_normalizes_key_hyphens(self) -> None:
        key = "my-secret-key"
        expected = "my-value"
        with patch.dict(os.environ, {"UNDERWRITE_SECRET_MY_SECRET_KEY": expected}, clear=False):
            backend = EnvSecretsBackend()
            result = backend.get(key)
        assert result == expected
...
class TestVaultSecretsBackend:
    ...
    def test_get_calls_hvac_and_returns_value(self) -> None:
        ...
        mock_client.secrets.kv.v2.read_secret_version.return_value = {"data": {"data": {"value": "my-secret-value"}}}
        with patch.dict("sys.modules", modules):
            backend = VaultSecretsBackend(url="http://vault:8200", token="test-token")
            result = backend.get("my-key")
        assert result == "my-secret-value"
        mock_hvac.Client.assert_called_once_with(url="http://vault:8200", token="test-token")
        mock_client.secrets.kv.v2.read_secret_version.assert_called_once_with(path="my-key", mount_point="secret")
    ...
    def test_get_uses_default_mount_point(self) -> None:
        ...
        mock_client = Mock()
        mock_hvac.Client.return_value = mock_client
        mock_client.secrets.kv.v2.read_secret_version.return_value = {"data": {"data": {"value": "val"}}}
        with patch.dict("sys.modules", modules):
            backend = VaultSecretsBackend()
            backend.get("k")
        mock_client.secrets.kv.v2.read_secret_version.assert_called_once_with(path="k", mount_point="secret")
    ...
    def test_get_uses_custom_mount_point(self) -> None:
        ...
        with patch.dict("sys.modules", modules):
            ...
            backend.get("k")
        mock_client.secrets.kv.v2.read_secret_version.assert_called_once_with(path="k", mount_point="team-secrets")
    ...
    def test_get_falls_back_to_vault_token_env(self) -> None:
        ...
        mock_client.secrets.kv.v2.read_secret_version.return_value = {"data": {"data": {"value": "val"}}}
        with patch.dict("sys.modules", modules):
            with patch.dict(os.environ, {"VAULT_TOKEN": "env-token"}, clear=False):
                backend = VaultSecretsBackend()
                backend.get("k")
        mock_hvac.Client.assert_called_once_with(url="https://localhost:8200", token="env-token")
    ...
    def test_raises_on_missing_hvac_package(self) -> None:
        ...
        with patch.dict("sys.modules", {"hvac": None, "hvac.exceptions": None}):
            with patch("builtins.__import__") as mock_import:
                ...
                with pytest.raises(ImportError, match="VaultSecretsBackend requires hvac"):
                    backend.get("k")
...
class TestAwsSecretsBackend:
    ...
    def test_set_calls_put_secret_value(self) -> None:
        ...
        mock_client.put_secret_value.assert_called_once_with(SecretId="my-key", SecretString="my-val")
    ...
    def test_set_calls_create_secret_on_resource_not_found(self) -> None:
        ...
        mock_client.create_secret.assert_called_once_with(Name="new-key", SecretString="new-val")
