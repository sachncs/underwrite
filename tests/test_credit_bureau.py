...
class TestCreditBureauCheck:
    ...
    def test_check_bureau_emits_checked(self) -> None:
        bus = LocalBus()
        received: list = []
        bus.subscribe(EventType.CREDIT_BUREAU_CHECKED,
                      lambda e: received.append(e))
        s = svc(bus=bus)
        s._client = MockCreditBureauClient()
        s._client.add_report(
            "ABCDE1234F",
            CreditReport(bureau="cibil", pan="ABCDE1234F", name="A",
                         dob="1990-01-01", score=750))
        bus.start()
        s.handle(
            Event(event_type=EventType.CREDIT_BUREAU_CHECK,
                  source="test",
                  payload={
                      "pan": "ABCDE1234F",
                      "bureau": "cibil"
    ...
    def test_check_bureau_missing_pan_ignored(self) -> None:
        ...
        s.handle(
            Event(event_type=EventType.CREDIT_BUREAU_CHECK,
                  source="test",
                  payload={"bureau": "cibil"}))
        assert len(received) == 0

    def test_check_bureau_failure_emits_failed(self) -> None:
        bus = LocalBus()
        received: list = []
        bus.subscribe(EventType.CREDIT_BUREAU_CHECK_FAILED,
                      lambda e: received.append(e))
        s = svc(bus=bus)
    ...
    def test_get_report_after_check(self) -> None:
        ...
        s._client.add_report(
            "ABCDE1234F",
            CreditReport(bureau="cibil", pan="ABCDE1234F", name="Test",
                         dob="1990-06-15", score=720))
        s.handle(
            Event(event_type=EventType.CREDIT_BUREAU_CHECK,
                  source="test",
                  payload={
    ...
    def test_multiple_bureau_checks(self) -> None:
        ...
        s._client.add_report(
            "PAN1",
            CreditReport(bureau="cibil", pan="PAN1", name="A",
                         dob="1990-01-01", score=750))
        s._client.add_report(
            "PAN2",
            CreditReport(bureau="experian", pan="PAN2", name="B",
                         dob="1991-02-02", score=680))
        s.handle(
            Event(event_type=EventType.CREDIT_BUREAU_CHECK,
                  source="test",
                  payload={
...
class TestCkycVerification:
    ...
    def test_verify_ckyc_emits_verified(self) -> None:
        ...
        mock = MockCreditBureauClient()
        s._client = mock
        mock.add_ckyc("CKYC1234567890",
                       CkycResponse(ckyc_number="CKYC1234567890",
                                    name="Test User", dob="1990-01-01",
                                    gender="M", pan="ABCDE1234F",
                                    aadhaar_verified=True, address="Addr",
                                    status="verified"))
        bus.start()
        s.handle(
            Event(event_type=EventType.CKYC_VERIFY,
                  source="test",
                  payload={
    ...
    def test_verify_ckyc_missing_fields_ignored(self) -> None:
        ...
        s.handle(
            Event(event_type=EventType.CKYC_VERIFY,
                  source="test",
                  payload={"ckyc_number": "CKYC1234567890"}))
        s.handle(
            Event(event_type=EventType.CKYC_VERIFY,
                  source="test",
                  payload={"aadhaar": "1234"}))
        assert len(received) == 0
    ...
    def test_get_ckyc_after_verify(self) -> None:
        ...
        mock = MockCreditBureauClient()
        s._client = mock
        mock.add_ckyc("CKYC9999999999",
                       CkycResponse(ckyc_number="CKYC9999999999",
                                    name="Jane Doe", dob="1985-05-20",
                                    gender="F", pan="ZYXWV9876K",
                                    aadhaar_verified=True, address="Addr",
                                    status="verified"))
        s.handle(
            Event(event_type=EventType.CKYC_VERIFY,
...
class TestMockCreditBureauClient:
    ...
    def test_fetch_report_not_found(self) -> None:
        from underwrite.services.credit_bureau.client import (
            CreditBureauNotFoundError)
        mock = MockCreditBureauClient()
        try:
            mock.fetch_credit_report("NOPAN")
            assert False, "expected CreditBureauNotFoundError"
        except CreditBureauNotFoundError:
            pass
    ...
    def test_fail_on(self) -> None:
        ...
        mock.add_report("PAN1",
                         CreditReport(bureau="cibil", pan="PAN1", name="A",
                                      dob="1990-01-01", score=750))
        try:
            mock.fetch_credit_report("PAN1")
            assert False, "expected CreditBureauError"
        except CreditBureauError:
            pass

    def test_fetch_report_success(self) -> None:
        mock = MockCreditBureauClient()
        report = CreditReport(bureau="cibil", pan="ABCDE1234F", name="A",
                              dob="1990-01-01", score=780,
                              active_accounts=3, delinquent_accounts=1)
    ...
    def test_verify_ckyc_success(self) -> None:
        mock = MockCreditBureauClient()
        resp = CkycResponse(ckyc_number="CKYC1234", name="Test",
